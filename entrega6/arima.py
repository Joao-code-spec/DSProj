...
from ts_functions import split_dataframe, HEIGHT

file_tag = 'diabetic'
index='Date'
target='glucose'
data = read_csv('entrega6/data/glucoseDateTarget.csv', index_col=[0], sep=',', decimal='.', parse_dates=True, infer_datetime_format=True, dayfirst=True)
#data.index.freq = 'H'
...
savefig(f'entrega6/catarina/{file_tag}_arima_study.png')
show()
...
plot_evaluation_results(train.values, prd_trn, test.values, prd_tst, f'entrega6/images/catarina/{file_tag}_arima_plot.png')
savefig( f'entrega6/catarina/{file_tag}_arima_eval.png')
plot_forecasting_series(train, test, prd_trn, prd_tst, f'entrega6/images/catarina/{file_tag}_arima_plot.png', x_label= str(index), y_label=str(target))
savefig( f'entrega6/catarina/{file_tag}_arima_plot.png')